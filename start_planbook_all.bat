...
start "PlanBook AI - Celery Worker" cmd /k "title PlanBook AI - Celery Worker && echo 🎯 PlanBook AI - Celery Worker && echo ⚡ Starting Celery Worker... && python -m celery -A app.core.celery_app worker --loglevel=info --pool=solo --concurrency=1 --include=app.tasks.pdf_tasks --queues=pdf_queue,default --hostname=planbook_worker@%%h"
