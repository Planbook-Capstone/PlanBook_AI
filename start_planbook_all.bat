...
start "PlanBook AI - Celery Worker" cmd /k "title PlanBook AI - Celery Worker && echo 🎯 PlanBook AI - Celery Worker && echo ⚡ Starting Celery Worker... && python -m celery -A app.core.celery_app worker --loglevel=info --pool=threads --concurrency=8 --queues=pdf_queue,embeddings_queue,cv_queue,slide_generation_queue,default --hostname=planbook_worker@%%h"
