...
class QdrantService:
    ...
    async def search_all_textbooks(self, query: str, limit: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            if not textbook_collections:
                return {
                    ...
                    "query": query,
                    "results": [],
                    "message": "No textbooks found in the system",
                }
            ...
            for collection_name in textbook_collections:
                try:
                    # Extract book_id từ collection name
                    ...
                    search_result = self.qdrant_client.search(
                        collection_name=collection_name,
                        query_vector=query_vector,
                        limit=limit,  # Lấy nhiều kết quả từ mỗi collection
                        with_payload=True,
                        score_threshold=0.3,
                    )
                    ...
                    logger.warning(
                        f"Error searching in collection {collection_name}: {e}"
                    )
                    continue

            # Sắp xếp theo score giảm dần và lấy top results
            ...
            return {
                ...
                "collections_searched": textbook_collections,
